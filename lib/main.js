...
const packageName = "relative-path-finder";
...
const fn = {
      ...
      self
        ...
        .then(function(searchPath) {
          ...
          for (var i = 0; i < entries.length; i++) {
            ...
            try {
              stats = fs.statSync(
                path.normalize(searchPath + path.sep + entry)
              );
              if (stats) {
  ...
  getReplacementPrefix: function(prefix, req) {
    var rPrefix =
      (rPrefix = prefix.match(pathReplacePrefixRegex)) !== null
        ? rPrefix[1]
        : prefix.trim();
    return rPrefix;
  },
  ...
  formatFile: function(file, prefix, req) {
    return {
      ...
      replacementPrefix: this.escapeSingleQuotedText(
        this.getReplacementPrefix(prefix, req),
        req
      ),
      type: "tag",
